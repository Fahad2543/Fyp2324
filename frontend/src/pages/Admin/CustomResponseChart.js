...
const CustomResponseChart = ({ data }) => {
  const [chart, setChart] = useState(null);
  const [filter, setFilter] = useState('day');

  useEffect(() => {CustomResponseChart
    if (chart) {
      chart.destroy();
    }

    const ctx = document.getElementById('CustomResponseChart');
...
const CustomDummyResponseChart = () => {
  return <CustomResponseChart data={dummyData} />;
};

export default CustomDummyResponseChart;
